...
//    variable to set a date selected for booking a volunteer
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        timePeriodSpinner = findViewById(R.id.timePeriodSpinner) // Time period to book the volunteer
        ...
        sharedPrefsManager = SharedPreferncesManager(this) // storing the using in memory - using shared preference
...
//            format the date for maintaining uniformity with the database and other activities in the project.
            ...
            selectedDateTextView.text = "Selected Date: $formattedDate" // show the date in UI, so that user can see their preferred date.

            // Query Firestore for available volunteers on the selected date
            queryAvailableVolunteers(formattedDate, volunteersAdapter) // Pass volunteersAdapter here
        }, year, month, day)
...
//                    get the userId of the user and pass it to "Users" table to get all the details of that user.
                    userId?.let { id ->
                        lifecycleScope.launch {
                            val volunteer = getVolunteerDetailsById(id) // call the getVolunteerDetailsById function on a given id
...
/**    Dialog box to show the details of the selected volunteer and confirm the booking */
    private fun showBookVolunteerDialog(volunteer: Volunteer) {
        ...
        dialogView.findViewById<TextView>(R.id.volunteerNameTextView).text ="Name: " + volunteer.name
        dialogView.findViewById<TextView>(R.id.volunteerAgeTextView).text = "Age: " + volunteer.age
        dialogView.findViewById<TextView>(R.id.volunteerContactTextView).text = "Contact: " + volunteer.contact
