...
class VolunteerSlotAvailabilityActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val currentFormattedDate = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(currentDate.time)
        ...
        button.setOnClickListener {
            val volunteerAvailability = VolunteerAvailability(userId.toString(), formattedSelectedDate, selectedSlots,"")
    ...
    private fun updateAvailabilityUI() {
        ...
        timeSlotTextViews.forEach { textView ->
            val slotText = textView.text.toString()
            if (selectedSlots.contains(slotText)) {
                // Slot is available, set background color to darker
                textView.isSelected = true;
            } else {
                // Slot is not available, set background color to light
